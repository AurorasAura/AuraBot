...
def quit(bot, trigger):
    ...
    if not quit_message:
        quit_message = 'I\'m downloading moar RAM %s' % trigger.nick
...
@sopel.module.priority('low')
@sopel.module.example('.msg #Channel/Nick Message')
def msg(bot, trigger):
    """
...
def invite_join(bot, trigger):
    """
    Join a channel Tempest is invited to, if the inviter is an admin.
    """
...
def hold_ground(bot, trigger):
    ...
    WARNING: This may not be needed and could cause problems if Tempest becomes
...
def mode(bot, trigger):
    """Set a user mode on Tempest. Can only be done in privmsg by an admin."""
...
def set_config(bot, trigger):
    """See and modify values of Tempest config object.
...
def save_config(bot, trigger):
    """Save state of Tempests config object to the configuration file."""
    bot.config.save()
